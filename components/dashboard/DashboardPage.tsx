...
const PieChart: React.FC<{ data: { name: string, value: number, color: string }[] }> = ({ data }) => {
  ...
  if (total === 0) return <div className="flex items-center justify-center h-48 text-gray-500">No data available</div>;
...
const BarChart: React.FC<{ data: { name: string, value: number }[], color: string }> = ({ data, color }) => {
  ...
  if (maxValue === 0) return <div className="flex items-center justify-center h-48 text-gray-500">No data available</div>;
...
const DashboardPage: React.FC<DashboardPageProps> = ({ students, teachers, classes, payments }) => {
  ...
  const genderPieData = [
    { name: 'Garçons', value: genderCounts.male, color: '#60a5fa' }, // blue-400
    { name: 'Filles', value: genderCounts.female, color: '#f472b6' } // pink-400
  ];
  ...
  const financialPieData = [
    { name: 'Payé', value: paymentStatusCounts.paid, color: '#4ade80' }, // green-400
    { name: 'Partiel', value: paymentStatusCounts.partial, color: '#facc15' }, // yellow-400
    { name: 'Non Payé', value: paymentStatusCounts.unpaid, color: '#f87171' } // red-400
  ];
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-gray-600">Aperçu en temps réel de votre établissement.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard icon={<UserGroupIcon />} title="Étudiants" value={totalStudents.toLocaleString()} color="border-blue-500" />
        <StatCard icon={<AcademicCapIcon />} title="Enseignants" value={totalTeachers.toLocaleString()} color="border-green-500" />
        <StatCard icon={<LibraryIcon className="h-6 w-6"/>} title="Classes" value={totalClasses.toLocaleString()} color="border-yellow-500" />
        <StatCard icon={<CurrencyDollarIcon />} title="Revenus Collectés (TND)" value={totalRevenue.toLocaleString('fr-FR', {minimumFractionDigits: 2})} color="border-purple-500" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 bg-white p-6 rounded-xl shadow-md">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">Distribution des Étudiants par Niveau</h2>
          <BarChart data={levelChartData} color="bg-blue-400" />
        </div>

        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded-xl shadow-md">
            <h2 className="text-xl font-semibold text-gray-700 mb-4">Répartition par Genre</h2>
            <PieChart data={genderPieData} />
          </div>
          <div className="bg-white p-6 rounded-xl shadow-md">
            <h2 className="text-xl font-semibold text-gray-700 mb-4">Aperçu Financier</h2>
