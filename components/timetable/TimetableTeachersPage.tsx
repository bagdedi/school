...
const TimetableTeachersPage: React.FC<TimetableTeachersPageProps> = ({ workingHours, schoolName }) => {
  ...
  const teachers = useMemo(() => [...new Set(mockEvents.map(e => e.teacher))].sort(), []);
  ...
  const printTitle = selectedTeacher 
    ? `Emploi du temps: ${selectedTeacher}` 
    : "Emploi du temps: Tous les enseignants";
